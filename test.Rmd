...
We will analyze the association of hypertension and other factors in our study. 
...
1. [National Health and Nutrition Examination Survey (NHANES)](https://wwwn.cdc.gov/nchs/nhanes/default.aspx): A program of studies designed to assess the health and nutritional status of adults and children in the United States.The survey is unique in that it combines interviews and physical examinations. It started in the 1960s and became a continuous program starting in 1999. Data from this survey will be used in epidemiological studies and health sciences research. It help develop sound public health policy, direct and design health programs and services, and expand the health knowledge for the Nation.
NYC HANES is a local version of NHANES, which implies it mainly focus on New York area. Modeling on NHANES, NYC HANES collected data from a physical examination and laboratory tests, as well as a face-to-face and an audio computer-assisted self-interview. NHANES results have led to many important health programs and policy changes. 
...
DF$race <- factor(DF$race, levels=c(100, 110, 120, 140, 180, 250), 
                               labels=c('White', 'Black/African American', 'Indian (American)/Alaska Native', 
                                        'Native Hawaiian/Other Pacific Islander', 'Asian', 'Some Other Race'))
...
DF$income <-  factor(DF$income, levels=c(1:6), 
                                labels=c('Less than $20,000','$20,000 - $39,999','$40,000 - $59,999',
...
p1 <- ggplot(DF, aes(x = hypertension, y=age))+
  geom_boxplot()+ggtitle('Relationship between hypertension and age')
p2 <- ggplot(DF, aes(x = hypertension, y=gender))+ 
  geom_boxplot()+ggtitle('Relationship between hypertension and gender')
ggarrange(p1,p2)
```
...
ggplot(DF, aes(x = hypertension, fill = gender)) + 
  geom_bar()+ggtitle('Relationship between hypertension and gender')
```
...
ggplot(DF, aes(x = hypertension, fill = cholesterol)) + 
  geom_bar(position = "fill")+ggtitle('Relationship between hypertension and cholesterol')
```
...
```{r}
g1 <- glm(hypertension~ .-id-surveyweight-hypertension, family = binomial(link = "logit"),data = DF)
#summ(g1,exp=TRUE)
```
...
g2 <- svyglm(hypertension ~ age+race+gender+diet+income+diabetes+bmi+cholesterol+drink+smoking, 
...
```{r}
g3 <- svyglm(hypertension ~ age+gender+diet+diabetes+bmi+cholesterol+drink+smoking, 
               family = binomial(link = "logit"), design = hypertension_design)

s <- data.frame('c'=exp(g3$coefficients[2:16]),
                'l'=seq(from=2, to=16, by=1),
                'n'=names(g3$coefficients)[2:16],
                'z'=10*seq(from=2, to=16, by=1))
