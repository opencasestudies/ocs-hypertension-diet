...
The website (http://nychanes.org/data/) provides guidelines 
about how to use weights for different purposes, and you 
can find them through the 
[Analytics Guideline](http://nychanes.org/wp-content/uploads/sites/6/2015/11/ANALYTIC-GUIDELINES-2016_V2.pdf). 

As the weight is given in the original dataset, we can use 
the variable 'surveyweight' directly in our analysis. Here,
...
[Analytics Guideline](http://nychanes.org/wp-content/uploads/sites/6/2015/11/ANALYTIC-GUIDELINES-2016_V2.pdf). Next let's calculate the FPC as below:
...
the R package `survey`. This function combines a data 
frame and all the design information needed to specify 
...
you can set the 
[arguments](https://www.rdocumentation.org/packages/jtools/versions/2.0.0/topics/set_summ_defaults) to be used in every call to `summ()` 
...
for 'Former smoker' is 0.56, which means 'Former smoker' 
is associated with an odds of getting hypertension of 56% 
...
export_summs(g, g_svy, exp = TRUE, 
             model.names = c('glm', 'svyglm'))
```

`export_summs()` in package `huxtable` is designed to give 
...
rise. Therefore, compared with those who never smoke, the 
odds of getting hypertension for former smokers and current
smokers also change to 0.67 and 0.73 respectively; Second, 
...
The function `plot_summs()`, which is provided by library 
`jtools`, plots regression coefficients and their associated
...
The label names are default setting, but to make it more readable,
...
'very good diet'='dietVery good','excellent diet'='dietExcellent'),
 model.names = c("svyglm", "glm"), 
scale=TRUE)+ xlab('95% confidence interval')
...
```
Obviously, 'cholesterol', 'diabetes', 'smoke' and 'drink' 
...
the confidence interval computed by `svyglm` (ci_svyglm) 
is larger than that computed by `glm` (ci_glm). It is strange, 
...
for 'cholesterol', 'diabetes' ,'smoke' and 'drink' have obvious 
