...
```{r}
res$term <- c("intercept", 'age' ,'bmi','gender',
'cholesterol','current smoker',
'diabetic with diagnosis', 'former smoker', 
'good diet','fair diet','very good diet', 
'excellent diet', 'diabetic without diagnosis'
) 

#reorder the dataframe based on length of confidence interval
res <- res[order(res$len.x),]

res[,c("term","len.x","len.y")]
```
`len.x` is the confidence interval constructed by `svyglm()`
(`ci_svyglm`) while `len.y` is constructed by `glm()`(`ci_glm`)
...
ggplot(res[-13,]) +
     ...
     geom_errorbarh(aes(estimate.x, term, xmin = low.x, 
     xmax = high.x, height = 0.5), size = 0.8, color = "red")+
     geom_errorbarh(aes(estimate.y, term, xmin = low.y, 
     xmax = high.y, height = 0.5), size = 0.8)+
     theme(axis.title.x = element_blank(),
     axis.title.y = element_blank()) +
    ggtitle(expression(atop(bold("95% condfidence interval"), 
                            atop(italic("red:svyglm  black:glm")))))
```
...
The red one is `ci_svyglm` while the black one is `ci_glm`.
