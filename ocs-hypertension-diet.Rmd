...
[Variable Codebook](http://nychanes.org/wp-content/uploads/sites/6/2019/01/28283961_NYC-HANES_codebook_Public_V3_011019.pdf) again, we found another variable `AlQ_1` (how often did the survey participant drink any type of alcoholic beverage), where 0 means they never drink.
...
[Variable Codebook](http://nychanes.org/wp-content/uploads/sites/6/2019/01/28283961_NYC-HANES_codebook_Public_V3_011019.pdf) to get the correct mapping of the numerical values 
...
p1 <- ggplot(hy_df, aes(x = hypertension, y=age))+
...
p2 <- ggplot(hy_df, aes(x = hypertension, y=gender))+ 
  geom_boxplot()+ggtitle('distribution of gender')
p3 <- ggplot(hy_df, aes(x = gender, fill = hypertension)) + 
  geom_bar()+ggtitle('distribution of hypertension')
p4 <- ggplot(hy_df, aes(x = gender, fill = hypertension)) + 
...
p5 <- ggplot(hy_df, aes(x = smoking, fill = drink)) + 
  geom_bar(position = "fill")+ggtitle('smoking and drink') + ylab('proportion')
p6 <- ggplot(hy_df, aes(x = cholesterol, fill = diabetes)) + 
...
```{r}
p7 <- ggplot(hy_df, aes(x = cholesterol, y = bmi, fill = cholesterol)) +
...
+ `weights= ~hy_df$surveyweight` tells it where to find the weight.
...
g_svy <- glm(hypertension~ smoking, 
          family = binomial(link = 'logit'),data = hy_df)
