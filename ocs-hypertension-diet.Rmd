...
participant responses to all interview questions. All of our 
...
* `data = hy_df[,-c(1,13)]` tells `svydesign` where to find the variables for analysis
...
We see that `cholesterol`, `diabetes`, `smoking` and `diet`
have larger differences in confidence intervals comparing 
the standard glm to the survey-weighted glm results, while 
`age`, `bmi` and `gender` have only small differences. We also see that, in general, the confidence intervals from `ci_svyglm` are larger than those from `ci_glm`. So we would not get the "correct" results if we failed to take into account the survey weighting in our analysis.
