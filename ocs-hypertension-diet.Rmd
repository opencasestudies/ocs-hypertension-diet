...
|`jtools`|Provides some functions to understand and sharethe results of (primarily) regression analyses efficiently|
|`plotrix`|Lots of plots, various labeling, axis and color scaling functions|
|`huxtable`|An R package to create LaTeX and HTML tables, with a friendly, modern interface|
...
```{r}
summ(g,exp = TRUE,model.info = FALSE, model.fit = FALSE)
```
...
for 'Former smoker' is `r summ(g,exp = TRUE)$coeftable[2,1]`, 
...
of `r summ(g1,exp = TRUE)$coeftable[25,1]` of that for 'Never smoker'; 
'Current smoker' is associated with an odd of getting hypertension 
of `r summ(g1,exp = TRUE)$coeftable[26,1]` of that for 'Never smoker'. 

* `bmi`: Holding all other variables the same, a unit increase in bmi 
changes odds by `r summ(g1,exp = TRUE)$coeftable[20,1]`.
...
smokers also change to `r summ(g_svy,exp = TRUE)$coeftable[2,1]` 
and `r summ(g_svy,exp = TRUE)$coeftable[3,1]` respectively; Second, 
...
'gender' = 'gender',
'former smoker' = 'smokingFormer smoker',
'current smoker' = 'smokingCurrent smoker',
'fair diet' = 'dietFair','good diet' = 'dietGood',
'very good diet' = 'dietVery good',
'excellent diet' = 'dietExcellent'),
 model.names = c("svyglm", "glm"), 
...
```
Obviously, `cholesterol`, `diabetes`, `smoking` and `drink`
...
for `cholesterol`, `diabetes`,`smoking` and `drink` have obvious 
