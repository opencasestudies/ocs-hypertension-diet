...
MAT: Just use relative paths inside the project
```{r}
...
For the covariates of interest, we check the related paper and selected 11 covariates that have a potential association with hypertension according to the authors. The variables can also be chosen by univariate analysis in the whole data set with adjusted family error rates. We will talk about the way of doing that in the other project.
...
MAT: The "glimpse" function from the tidyverse is also useful for this.
```{r}
...
```{r}
source(paste0(directory, "/svyplot.R"))
```
